...
void Session_AddSample(float temp, float humidity,
    ...
    // MCXC444 uses 12-bit ADC (0-4095)
...
void Session_StorePendingReport(const SessionSummary_t *s) {
    sPendingSummary = *s;        /* write struct first  */
    sPendingReport  = true;      /* then set flag       */
    Serial.println("[Session] Report queued for Gemini analysis");
}

bool Session_ConsumePendingReport(SessionSummary_t *out) {
    if (!sPendingReport) return false;
    *out           = sPendingSummary;
    sPendingReport = false;
    return true;
}